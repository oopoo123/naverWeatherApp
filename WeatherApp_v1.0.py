...
import threading
import time
## PyQt5Designer 다운 UI만들기

from_class = uic.loadUiType("ui/weatherApp.ui")[0]

class WeatherWin(QMainWindow, from_class):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.setWindowTitle("오늘의 날씨")
        self.setWindowIcon(QIcon("icon/wicon"))
    ...
    def request_weather(self):
        ...
        try:
            ...
            sense_temperature = weather_soup.find('div', {'class': 'weather_info'}).find('dl', {
                'class': 'summary_list'}).find('dd', {'class': 'desc'}).text  # 체감온도
    ...
    def reflashTimer(self): # 자동 갱신
        ...
        threading.Timer(60, self.reflashTimer).start() # 60초 마다 자동 갱신하게 하는 타이머
